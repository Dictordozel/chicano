...
<section class="mx-auto max-w-3xl px-4 py-12 sm:px-6 sm:py-20">
	<div class="text-center">
		<p class="display text-[0.6rem] text-gold">{data.user.email}</p>
		...
		<ul class="mt-2 divide-y divide-zinc-900">
			{#each data.upcoming as a (a.id)}
				<li class="flex flex-wrap items-center gap-4 py-5">
					...
					>
						<span class="display text-[0.5rem] text-zinc-500">{fmt(a.date).split(' ')[0]}</span>
						<span class="gothic text-2xl text-gold tabular-nums">{a.date.slice(8)}</span>
						<span class="display text-[0.5rem] text-gold tabular-nums">{a.time}</span>
					</div>

					<div class="min-w-0 flex-1">
						<p class="display text-[0.68rem] text-zinc-100">{a.service}</p>
						<p class="mt-1.5 text-[0.8rem] text-zinc-500">
							{a.barber_alias} — {a.barber_name} · {a.duration_min} min
						</p>
						{#if a.note}
							<p class="mt-1.5 text-[0.75rem] text-zinc-600 italic">“{a.note}”</p>
						{/if}
					</div>
					...
					<div class="flex items-center gap-4">
						...
						<form method="POST" action="?/cancel" use:enhance={onCancel}>
							...
							<button
								type="submit"
								class="display text-[0.55rem] text-zinc-600 transition-colors hover:text-flash"
							>
								Cancel
							</button>
						</form>
	...
	{#if data.past.length > 0}
		<h2 class="display mt-16 border-b border-zinc-800 pb-4 text-[0.65rem] text-zinc-100">
			History
		</h2>
		<ul class="mt-2 divide-y divide-zinc-900">
			{#each data.past as a (a.id)}
				<li class="flex items-center gap-4 py-4 text-sm">
					<span class="w-28 shrink-0 text-[0.72rem] text-zinc-600 tabular-nums">
						{fmt(a.date)} · {a.time}
					</span>
					...
					<span
						class="display shrink-0 text-[0.5rem] {a.status === 'cancelled'
							? 'text-flash'
							: 'text-zinc-600'}"
					>
						{a.status}
			...
			{#each data.orders as order (order.id)}
				<li class="py-5">
					<div class="flex items-baseline justify-between gap-4">
						<span class="display text-[0.6rem] text-zinc-300">Order #{order.id}</span>
						<span class="gothic text-xl text-gold tabular-nums">{order.total} ₽</span>
					</div>
					<ul class="mt-2 space-y-1">
						{#each order.items as item (item.id)}
							<li class="flex justify-between gap-4 text-[0.78rem] text-zinc-500">
