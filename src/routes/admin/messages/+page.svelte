...
<section class="mx-auto max-w-6xl px-4 py-10 sm:px-6 sm:py-14">
	...
	<!-- ------------------------------------------------- provider -->
	<div
		class="mt-6 flex flex-wrap items-center gap-3 border px-4 py-3.5 text-[0.9rem] {data.provider
			.ready
			? data.provider.live
	...
	>
		...
		<span class="display text-[0.75rem]">{data.provider.name}</span>
		<span class="flex-1">{data.provider.detail}</span>
		{#if !data.provider.live}
			<code class="text-[0.85rem] text-zinc-600">SMS_PROVIDER=smsru SMS_API_KEY=…</code>
		{/if}
	</div>
	...
	<dl class="mt-3 grid grid-cols-2 gap-3 sm:grid-cols-5">
		{#each [{ k: 'Queued', v: data.stats.byStatus.pending, hint: data.stats.nextAt ? `next ${when(data.stats.nextAt)}` : 'nothing due' }, { k: 'Sent', v: data.stats.byStatus.sent }, { k: 'Failed', v: data.stats.byStatus.failed }, { k: 'No phone', v: data.stats.byStatus.skipped }, { k: 'Expired', v: data.stats.byStatus.expired }] as row (row.k)}
			<div class="border border-zinc-800 bg-zinc-900/20 p-4">
				...
				{#if row.hint}<dd class="mt-0.5 text-[0.8rem] text-zinc-600">{row.hint}</dd>{/if}
			</div>
		{/each}
	</dl>
	...
	<div class="mt-4 flex flex-wrap gap-2">
		<form method="POST" action="?/drain" use:enhance={simple('Queue drained')}>
			<button type="submit" class="btn btn-ghost !px-4 !py-2 text-[0.75rem]">Send anything due now</button>
		</form>
		<form method="POST" action="?/resync" use:enhance={simple('Reminders rebuilt')}>
			<button type="submit" class="btn btn-ghost !px-4 !py-2 text-[0.75rem]">Rebuild reminders</button>
		</form>
	</div>

	<!-- ================================================ broadcast -->
	<h2 class="display mt-14 flex items-center gap-2 border-b border-zinc-800 pb-4 text-[0.85rem] text-gold">
	...
	<div class="mt-6 grid gap-8 lg:grid-cols-[minmax(0,1fr)_20rem]">
		<div>
			...
			<form method="GET" class="grid gap-3 sm:grid-cols-2">
				...
				<div class="sm:col-span-2">
					<button type="submit" class="btn btn-ghost !px-4 !py-2 text-[0.75rem]">
						Recount recipients
					</button>
				</div>
			</form>
			...
			<form method="POST" action="?/broadcast" use:enhance={onBroadcast} class="mt-6 space-y-4">
				...
				<div>
					...
					></textarea>
					<div class="mt-1.5 flex flex-wrap justify-between gap-3 text-[0.85rem]">
		...
		<aside class="card p-5">
			<p class="display text-[0.75rem] text-zinc-500">This selection</p>

			<p class="gothic mt-3 text-4xl text-gold tabular-nums">{data.preview.reachable}</p>
			<p class="text-[0.85rem] text-zinc-500">
			...
			{#if data.preview.unreachable.length > 0}
				<div class="mt-5 border-t border-zinc-800 pt-4">
					<p class="display text-[0.7rem] text-flash">
						{data.preview.unreachable.length} without a phone
					</p>
					<ul class="mt-2 space-y-1">
						{#each data.preview.unreachable.slice(0, 6) as u (u.email + (u.time ?? ''))}
							<li class="truncate text-[0.85rem] text-zinc-600">
								{u.name}{u.date ? ` · ${u.date} ${u.time}` : ''}
							</li>
						{/each}
					</ul>
					<p class="mt-2 text-[0.85rem] leading-relaxed text-zinc-600">
			...
			{#if data.preview.sample.length > 0}
				<div class="mt-5 border-t border-zinc-800 pt-4">
					<p class="display text-[0.7rem] text-zinc-500">Preview</p>
					<p class="mt-2 border border-zinc-800 bg-black/40 p-3 text-[0.85rem] leading-relaxed text-zinc-400">
						{data.preview.sample[0].body}
					</p>
					<p class="mt-1.5 text-[0.8rem] text-zinc-600">to {data.preview.sample[0].phone}</p>
				</div>
			{/if}
		</aside>
	</div>
	...
	<div class="mt-14 flex flex-wrap items-center justify-between gap-3 border-b border-zinc-800 pb-4">
		<h2 class="display text-[0.85rem] text-zinc-100">Outbox — {data.outbox.length} shown</h2>

		<form method="GET" class="flex flex-wrap items-center gap-2">
			<select name="status" class="field !w-auto !py-1.5 !text-[0.9rem]">
				{#each ['all', 'pending', 'sent', 'failed', 'skipped', 'expired', 'cancelled'] as s (s)}
					<option value={s} selected={data.statusFilter === s}>{s}</option>
				{/each}
			</select>
			<select name="kind" class="field !w-auto !py-1.5 !text-[0.9rem]">
		...
		<ul class="divide-y divide-zinc-900">
			{#each data.outbox as m (m.id)}
				<li class="flex flex-wrap items-start gap-x-4 gap-y-2 py-4">
					<span class="display w-24 shrink-0 pt-0.5 text-[0.7rem] {STATUS_COLOR[m.status] ?? 'text-zinc-500'}">
						{m.status}
					</span>

					<div class="min-w-0 flex-1">
						<p class="text-[0.95rem] leading-relaxed text-zinc-300">{m.body}</p>
	...
	<p class="mt-8 text-center text-[0.9rem] leading-relaxed text-zinc-600">
